...
class ParserSpec extends TestSpecification {

  "The grammar parser" should "parse simple rules" in {
    val rules = Table(
      "grammar" -> "structure",
    ...
    forAll(rules) { (grammar, structure) =>
      parse(grammar, GrammarParser.grammar(_)) should matchPattern { case Parsed.Success(`structure`, _) => }
    }
  ...
  it should "parse string escapes correctly" in {
    ...
    forAll(escapes) { (grammar, expected) =>
      inside(parse(grammar, GrammarParser.grammar(_))) { case Parsed.Success(Seq(("S", Literal(actual,_))), _) =>
  ...
  it should "ignore comments" in {
    ...
    val nt = 'S := 'a
    forAll(comments) {
      parse(_, GrammarParser.grammar(_)) should matchPattern { case Parsed.Success(Seq(`nt`), _) => }
    }
  }
  ...
  it should "parse regular expressions correctly" in {
    ...
    forAll(regexes) { (grammar, expected) =>
      inside(parse(grammar, GrammarParser.grammar(_))) {
        case Parsed.Success(Seq(("S", Regex(actual, _, _))), _) => actual shouldEqual expected
  ...
  it should "fail on incorrect inputs" in {
    ...
    forAll(fails) { nonGrammar =>
      parse(nonGrammar, GrammarParser.grammar(_)) should matchPattern { case Parsed.Failure(_, _, _) => }
    }
  }
}
