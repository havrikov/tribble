...
private[tribble] object GrammarParser {
  ...
  private def num[_: P]: P[Int] = CharsWhile(('0' to '9').contains(_)).!.map(_.toInt)
  ...
  private def concatenation[_: P]: P[DerivationRule] = atom.rep(1).map { case Seq(e) => e case seq => Concatenation(seq) }

  private def alternation[_: P]: P[DerivationRule] = concatenation.rep(min = 1, sep = "|"./).map(_.distinct).map { case Seq(e) => e case seq => Alternation(seq.toSet) }
  private def production[_: P]: P[Production] = P(reference ~ "=" ~/ alternation ~ ";").map { case (Reference(name, _), rhs) => name -> rhs }

  def grammar[_: P]: P[Seq[Production]] = P(Start ~ production.rep(1) ~ End)
}
